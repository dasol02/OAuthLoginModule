...
+ (void)show {
   //  [SVProgressHUD showGIFProgress:@"gears" maskType:SVProgressHUDDefaultMaskType];
...
+ (void)showLoadingImage {
    [[self sharedView] showLoadingImageProcess];
}
...
- (void)showLoadingImageProcess {
   ...
   // self.imageView.image = [UIImage sd_animatedGIFNamed:imageName];
...
- (void)updatePosition {
    ...
    CGFloat hudWidth = 110.0f;
    CGFloat hudHeight = 110.0f;
...
- (UIImageView *)imageView {
    if (!_imageView)
        _imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 64.0f, 110.0f)];
